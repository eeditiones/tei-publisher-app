...
declare function deploy:store-libs($target as xs:string, $userData as xs:string+, $permissions as xs:string) {
    ...
    for $lib in ("map.xqm", "facets.xqm", "annotation-config.xqm", "nlp-config.xqm", xmldb:get-child-resources($path)[starts-with(., "navigation")],
...
declare function deploy:create-app($collection as xs:string, $json as map(*)) {
    ...
    let $created := (
        ...
        deploy:copy-resource($collection, $base, "index.xqm", ($json?owner, "tei"), "rw-r--r--"),
