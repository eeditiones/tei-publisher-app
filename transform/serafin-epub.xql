    ...
    $input !         (
            ...
            return
                    ...
                    case element(anchor) return
                        epub:note($config, ., ("tei-anchor"), let $target := '#' || @xml:id return root(.)//div[@type='notes']/note[@target=$target], (), ())
                    ...
                    case element(seg) return
                        html:webcomponent($config, ., css:get-rendition(., ("tei-seg2")), ., 'pb-highlight', map {"key": replace(@xml:id, "^\w\.(.*)$", "$1")})
                    ...
                    case element(lb) return
                        html:omit($config, ., css:get-rendition(., ("tei-lb")), .)
