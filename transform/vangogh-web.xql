            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?view='metadata') then
                            html:block($config, ., ("tei-teiHeader5", "metadata"), .//sourceDesc//vg:letDesc)
                        else
                            if ($parameters?header='short') then
                                html:block($config, ., ("tei-teiHeader6"), .)
                            else
                    ...
                    case element(anchor) return
                        html:note($config, ., ("tei-anchor"), let $n := @n return $get(.)/ancestor::TEI//div[@type='notes']//note[@n=$n]/node(), (), @n/string())
                    ...
                    case element(lb) return
                        if ($parameters?view='normalized') then
                            ...
                            html:break($config, ., css:get-rendition(., ("tei-lb3")), ., 'line', @n)
