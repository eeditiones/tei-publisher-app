            ...
            return
                    ...
                    case element(author) return
                        if (ancestor::teiHeader) then
                            html:omit($config, ., ("tei-author1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                    ...
                    case element(formula) return
                        if (@rendition='simple:display') then
                            ...
                            html:inline($config, ., ("tei-formula2", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                    ...
                    case element(head) return
                        if (parent::figure) then
                            html:block($config, ., ("tei-head1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            if (parent::table) then
                                ...
                                if (parent::lg) then
                                    ...
                                    if (parent::list) then
                                        html:listItem($config, ., ("tei-head4", css:map-rend-to-class(.)), .)                                        => model:map($node, $trackIds)
                                    else
                                        if (parent::div) then
                                            html:heading($config, ., ("tei-head5", css:map-rend-to-class(.)), ., count(ancestor::div))                                            => model:map($node, $trackIds)
                                        else
                    ...
                    case element(note) return
                        if (@place) then
                            html:note($config, ., ("tei-note1", css:map-rend-to-class(.)), ., @place, @n)                            => model:map($node, $trackIds)
                        else
                            if (parent::div and not(@place)) then
                                html:block($config, ., ("tei-note2", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                            else
                                if (not(@place)) then
                                    html:inline($config, ., ("tei-note3", css:map-rend-to-class(.)), .)                                    => model:map($node, $trackIds)
                                else
                                    $config?apply($config, ./node())
                    ...
                    case element(fileDesc) return
                        html:title($config, ., ("tei-fileDesc", css:map-rend-to-class(.)), titleStmt)                        => model:map($node, $trackIds)
                    case element(profileDesc) return
                        html:omit($config, ., ("tei-profileDesc", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    ...
                    case element(teiHeader) return
                        html:metadata($config, ., ("tei-teiHeader", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                        ...
                        if (parent::titleStmt/parent::fileDesc) then
                            (
                                if (preceding-sibling::title) then
                                    html:text($config, ., ("tei-title1", css:map-rend-to-class(.)), ' — ')                                    => model:map($node, $trackIds)
                                else
                                    (),
                                html:inline($config, ., ("tei-title2", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                            )
                                ...
                                if (@level='m' or not(@level)) then
                                    (
                                        ...
                                        if (ancestor::biblFull) then
                                            html:text($config, ., ("tei-title5", css:map-rend-to-class(.)), ', ')                                            => model:map($node, $trackIds)
                                        else
                                            ()
                                    )
