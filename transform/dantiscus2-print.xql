            ...
            return
                    ...
                    case element(fileDesc) return
                        if ($parameters?mode='commentary') then
                            fo:block($config, ., ("tei-fileDesc1"), .)
                        else
                            if ($parameters?header='short') then
                                (
                                    fo:block($config, ., ("tei-fileDesc2", "header-short"), titleStmt),
                                    fo:block($config, ., ("tei-fileDesc3", "header-short"), editionStmt),
                                    fo:block($config, ., ("tei-fileDesc4", "header-short"), publicationStmt)
                                )

                            else
                                fo:title($config, ., ("tei-fileDesc5"), titleStmt)
                    case element(seg) return
                        fo:inline($config, ., css:get-rendition(., ("tei-seg2")), .)
                    case element(profileDesc) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        fo:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc1"), $content)
                        else
                            fo:omit($config, ., ("tei-profileDesc2"), .)
                    ...
                    case element(editor) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        fo:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-editor1"), $content)
                        else
                            if (preceding-sibling::editor) then
                                (
                                    fo:inline($config, ., ("tei-editor2"), ', '),
                                    fo:inline($config, ., ("tei-editor3"), .)
                                )
                    ...
                    case element(ref) return
                        if (@ana) then
                            ...
                            fo:inline($config, ., ("tei-ref1", "itinerary"), .)
                        else
                            if (not(@target)) then
                                fo:inline($config, ., ("tei-ref2"), .)
                            else
                                if (not(node())) then
                                    fo:link($config, ., ("tei-ref3"), @target, @target, map {})
                                else
                                    fo:link($config, ., ("tei-ref4"), ., @target, map {})
                    ...
                    case element(bibl) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        fo:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-bibl1"), $content)
                        else
                            if (parent::listBibl) then
                                fo:listItem($config, ., ("tei-bibl2"), ., ())
                            else
                    ...
                    case element(titleStmt) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        fo:block(map:merge(($config, map:entry("template", true()))), ., ("tei-titleStmt1", "editors"), $content)
                        else
                            fo:heading($config, ., ("tei-titleStmt3"), ., ())
                    ...
                    case element(sourceDesc) return
                        ...
                        return
                                                fo:block(map:merge(($config, map:entry("template", true()))), ., ("tei-sourceDesc"), $content)
                    case element(msDesc) return
                        ...
                        return
                                                fo:block(map:merge(($config, map:entry("template", true()))), ., ("tei-msDesc"), $content)
                    case element(person) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        fo:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-person1", "person-entry"), $content)
                        else
                    ...
                    case element(place) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        fo:block(map:merge(($config, map:entry("template", true()))), ., ("tei-place1"), $content)
                        else
