            ...
            return
                    ...
                    case element(pb) return
                        if (starts-with(@facs, 'iiif:')) then
                            html:webcomponent($config, ., ("tei-pb1", "facs", css:map-rend-to-class(.)), @n, 'pb-facs-link', map {"facs": replace(@facs, '^iiif:(.*)$', '$1')})
                        else
                    ...
                    case element(choice) return
                        if (sic and corr) then
                            html:alternate($config, ., ("tei-choice4", "choice", css:map-rend-to-class(.)), ., corr[1], sic[1], map {})
                        else
                            if ($parameters?mode='norm' and abbr and expan) then
                                html:alternate($config, ., ("tei-choice5", "choice", css:map-rend-to-class(.)), ., expan[1], abbr[1], map {})
                            else
                                if (abbr and expan) then
                                    (
                                        html:alternate($config, ., ("tei-choice6", "choice", css:map-rend-to-class(.)), ., abbr[1], expan[1], map {}),
                                        html:alternate($config, ., ("tei-choice7", "choice-alternate", css:map-rend-to-class(.)), ., expan[1], abbr[1], map {})
                                    )

                                else
                                    if (orig and reg) then
                                        html:alternate($config, ., ("tei-choice8", "choice", css:map-rend-to-class(.)), ., reg[1], orig[1], map {})
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(note) return
                        if (parent::person or parent::place) then
                            ...
                            if (@place) then
                                html:note($config, ., ("tei-note2", css:map-rend-to-class(.)), ., @place, @n)
                            else
                    ...
                    case element(rs) return
                        html:alternate($config, ., ("tei-rs", "name", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root($parameters?root))/node(), map {})
                    ...
                    case element(name) return
                        html:alternate($config, ., ("tei-name", "name", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root($parameters?root))/node(), map {})
