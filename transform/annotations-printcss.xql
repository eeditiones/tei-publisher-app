...
module namespace model="http://www.tei-c.org/pm/models/annotations/printcss";
            ...
            return
                    ...
                    case element(place) return
                        (
                            ...
                            html:block($config, ., ("tei-place4", css:map-rend-to-class(.)), note/node())
                        )

                    case element(person) return
                        (
                            html:heading($config, ., ("tei-person1", css:map-rend-to-class(.)), persName[@type="full"], 3),
                            html:block($config, ., ("tei-person2", css:map-rend-to-class(.)), string-join(occupation, ', ')),
