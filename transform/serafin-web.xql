    ...
    $input !         (
            ...
            return
                    ...
                    case element(anchor) return
                        html:note($config, ., ("tei-anchor", css:map-rend-to-class(.)), let $target := '#' || @xml:id return root(.)//div[@type='notes']/note[@target=$target], (), ())
                    ...
                    case element(note) return
                        if (parent::person) then
                            html:inline($config, ., ("tei-note", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(seg) return
                        html:webcomponent($config, ., css:get-rendition(., ("tei-seg2", css:map-rend-to-class(.))), ., 'pb-highlight', map {"key": replace(@xml:id, "^\w\.(.*)$", "$1")})
                    ...
                    case element(date) return
                        if (@when) then
                            html:alternate($config, ., ("tei-date3", css:map-rend-to-class(.)), ., ., format-date(xs:date(@when), '[D1o] [MNn] [Y]', (session:get-attribute('lang'), 'en')[1], (), ()), map {})
                        else
                            if (text()) then
                    ...
                    case element(persName) return
                        if (parent::person) then
                            ...
                            html:alternate($config, ., ("tei-persName4", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root(.)), map {})
                    ...
                    case element(placeName) return
                        if (parent::place) then
                            html:inline($config, ., ("tei-placeName3", css:map-rend-to-class(.)), .)
                        else
                            html:alternate($config, ., ("tei-placeName4", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root(.)), map {})
                    case element(orgName) return
                        if (parent::org) then
                            html:inline($config, ., ("tei-orgName2", css:map-rend-to-class(.)), .)
                        else
                            html:alternate($config, ., ("tei-orgName3", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root(.)), map {})
