...
module namespace model="http://www.tei-c.org/pm/models/serafin/printcss";
            ...
            return
                    ...
                    case element(choice) return
                        if (sic and corr) then
                            printcss:alternate($config, ., ("tei-choice4", css:map-rend-to-class(.)), ., corr[1], sic[1])
                        else
                            if (abbr and expan) then
                                printcss:alternate($config, ., ("tei-choice5", css:map-rend-to-class(.)), ., expan[1], abbr[1])
                            else
                                if (orig and reg) then
                                    printcss:alternate($config, ., ("tei-choice6", css:map-rend-to-class(.)), ., reg[1], orig[1])
                                else
                                    $config?apply($config, ./node())
                    ...
                    case element(persName) return
                        if ($mode='register') then
                            html:cell($config, ., ("tei-persName3", "persName", css:map-rend-to-class(.)), ., ())
                        else
                            if (parent::person) then
                                ...
                                printcss:alternate($config, ., ("tei-persName5", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root(.))/persName)
                    ...
                    case element(placeName) return
                        if ($mode='register') then
                            html:cell($config, ., ("tei-placeName3", "placeName", css:map-rend-to-class(.)), ., ())
                        else
                            if (parent::place) then
                                ...
                                printcss:alternate($config, ., ("tei-placeName5", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root(.))/placeName)
                    case element(orgName) return
                        if ($mode='register') then
                            html:cell($config, ., ("tei-orgName2", css:map-rend-to-class(.)), ., ())
                        else
                            if (parent::org) then
                                ...
                                printcss:alternate($config, ., ("tei-orgName4", css:map-rend-to-class(.)), ., ., id(substring-after(@ref, '#'), root(.))/orgName)
