            ...
            return
                    ...
                    case element(note) return
                        if ($parameters?view='notes') then
                            html:listItem($config, ., ("tei-note1"), ., @n)
                        else
                    ...
                    case element(c) return
                        if (@type='kk') then
                            html:inline($config, ., ("tei-c1"), ',')
                        else
                            if (@type='kp') then
                                html:inline($config, ., ("tei-c2"), '.')
                            else
                                html:inline($config, ., ("tei-c3"), .)
                    ...
                    case element(vg:letDesc) return
                        (
                            ...
                            epub:block($config, ., ("tei-vg_letDesc2"), note[@type='sourceStatus']/node()),
                            html:heading($config, ., ("tei-vg_letDesc3"), 'Location:', 3),
                            epub:block($config, ., ("tei-vg_letDesc4"), note[@type='location']/node()),
                            html:heading($config, ., ("tei-vg_letDesc5"), 'Date:', 3),
                            epub:block($config, ., ("tei-vg_letDesc6"), note[@type='date']/node()),
                            html:heading($config, ., ("tei-vg_letDesc7"), 'Additional:', 3),
                            epub:block($config, ., ("tei-vg_letDesc8"), note[@type='additionalDetail']/node())
                        )

                    case element(vg:whiteline) return
                        epub:break($config, ., ("tei-vg_whiteline"), ., 'column', ())
