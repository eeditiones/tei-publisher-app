    ...
    $input !         (
            typeswitch(.)
                ...
                case element(graphic) return
                    ext-beamer:graphic($config, ., "graphic", ., @url, @width, @height, @scale, desc)
                case element(head) return
                    if (parent::div[@type='frame']) then
                        ext-beamer:frametitle($config, ., "head1", .)
                    else
                        latex:heading($config, ., "head2", .)
                case element(hi) return
                    ext-beamer:alert($config, ., "hi", .)
                ...
                case element(teiHeader) return
                    ext-beamer:metadata($config, ., "teiHeader", .)
                case element(g) return
                    if (not(text())) then
                        latex:glyph($config, ., "g1", .)
                    else
                ...
                case element(div) return
                    if (@type='block') then
                        ext-beamer:beamer-block($config, ., "div1", "block", *[not(self::head)], head)
                    else
                        if (@type='alert') then
                            ext-beamer:beamer-block($config, ., "div2", "alertblock", *[not(self::head)], head)
                        else
                            if (@type='frame') then
                                ext-beamer:frame($config, ., "div3", .)
                            else
                                latex:section($config, ., "div4", .)
                ...
                case element(TEI) return
                    ext-beamer:document($config, ., "TEI", .)
                ...
                case element(code) return
                    ext-latex:code($config, ., "code", ., @lang)
