            ...
            return
                    ...
                    case element(cit) return
                        if (@type='example') then
                            html:block($config, ., ("tei-cit1", "example"), .)
                        else
                            if (@type='translation') then
                                html:block($config, ., ("tei-cit2", "translation"), .)
                            else
                                $config?apply($config, ./node())
                            ...
                            if ($parameters?mode='metadata') then
                                ...
                                if ($parameters?header='short') then
                                    (
                                        ...
                                        html:block($config, ., ("tei-titleStmt7"), subsequence(title, 2)),
                                        html:block($config, ., ("tei-titleStmt8"), (author, editor))
                                    )

                                else
                                    html:block($config, ., ("tei-titleStmt9"), .)
                    ...
                    case element(entryFree) return
                        html:block($config, ., ("tei-entryFree"), .)
                    ...
                    case element(gen) return
                        html:alternate($config, ., ("tei-gen"), ., ., @norm, map {})
                    case element(pos) return
                        html:alternate($config, ., ("tei-pos"), ., ., @norm, map {})
                    case element(msDesc) return
                        (
                            (: retrieves the primary language :)
                            html:block($config, ., ("tei-msDesc1"), (root(.)//langUsage/language)),
                            (: retrieves the form of the source from taxonomy file :)
                            html:block($config, ., ("tei-msDesc2"), id(substring-after(root(.)//catRef[@scheme='#form']/@target, '#'), doc('/db/apps/tei-publisher/data/taxonomy.xml'))),
                            html:block($config, ., ("tei-msDesc3"), .)
                        )

                    case element(institution) return
                        html:inline($config, ., ("tei-institution"), .)
                    case element(respStmt) return
                        html:alternate($config, ., ("tei-respStmt"), ., persName, resp, map {})
