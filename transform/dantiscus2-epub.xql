            ...
            return
                    ...
                    case element(profileDesc) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc1"), $content)
                        else
                    ...
                    case element(titleStmt) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-titleStmt1", "editors"), $content)
                        else
                            if ($parameters?header='short') then
                                (
                                    html:link($config, ., ("tei-titleStmt4"), title[1], $parameters?doc, (), map {}),
                                    epub:block($config, ., ("tei-titleStmt5"), subsequence(title, 2)),
                                    epub:block($config, ., ("tei-titleStmt6"), editor),
                                    (: incipit :)
                                    epub:block($config, ., ("tei-titleStmt7", "incipit"), root(.)//seg[@type='incipit'])
                                )
                    ...
                    case element(sourceDesc) return
                        ...
                        return
                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-sourceDesc"), $content)
                    case element(msDesc) return
                        ...
                        return
                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-msDesc"), $content)
                    ...
                    case element(place) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-place1"), $content)
                        else
