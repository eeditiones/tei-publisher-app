...
module namespace model="http://www.tei-c.org/pm/models/ukraina/web";
            ...
            return
                    ...
                    case element(graphic) return
                        if (parent::facsimile) then
                            html:webcomponent($config, ., ("tei-graphic1", css:map-rend-to-class(.)), ., 'pb-facs-link', map {"facs": @url})                            => model:map($node, $trackIds)
                        else
                            html:graphic($config, ., ("tei-graphic2", css:map-rend-to-class(.)), ., @url, @width, @height, @scale, desc)                            => model:map($node, $trackIds)
                    ...
                    case element(titleStmt) return
                        if ($parameters?mode='title') then
                            html:heading($config, ., ("tei-titleStmt3", css:map-rend-to-class(.)), (title, author), ())                            => model:map($node, $trackIds)
                        else
                            if ($parameters?header='short') then
                                (
