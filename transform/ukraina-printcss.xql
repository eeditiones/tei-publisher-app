(:~
    ...
    ODD: /db/apps/tei-publisher/odd/ukraina.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/ukraina/printcss";
            ...
            return
                    ...
                    case element(graphic) return
                        if (parent::facsimile) then
                            html:webcomponent($config, ., ("tei-graphic1", css:map-rend-to-class(.)), ., 'pb-facs-link', map {"facs": @url})
                        else
                    ...
                    case element(titleStmt) return
                        if ($parameters?mode='title') then
                            html:heading($config, ., ("tei-titleStmt3", css:map-rend-to-class(.)), (title, author), ())
                        else
                            if ($parameters?header='short') then
                                (
