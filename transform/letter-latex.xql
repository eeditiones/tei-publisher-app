...
module namespace model="http://www.tei-c.org/pm/models/letter/latex";
            ...
            typeswitch(.)
                ...
                case element(name) return
                    if (@type='person' and @ref) then
                        (
                            latex:inline($config, ., ("tei-name1", "person"), .),
                            latex:note($config, ., ("tei-name2"), id(substring-after(./@ref, '#') ), (), count(preceding::name[@ref][@type='person'][ancestor::body]) + 1)
                        )
                ...
                case element(note) return
                    if (ancestor::person) then
                        ...
                        latex:note($config, ., ("tei-note2"), ., @place, ())
                ...
                case element(person) return
                    latex:inline($config, ., ("tei-person"), (persName, occupation, note))
                case element(persName) return
                    latex:inline($config, ., ("tei-persName"), string-join((text(), forename, surname[not(@type)]), ' '))
                case element() return
