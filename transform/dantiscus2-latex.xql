            ...
            return
                    ...
                    case element(profileDesc) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-profileDesc1"), $content)
                        else
                            latex:omit($config, ., ("tei-profileDesc2"), .)
                    ...
                    case element(editor) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-editor1"), $content)
                        else
                            if (preceding-sibling::editor) then
                                (
                                    latex:inline($config, ., ("tei-editor2"), ', '),
                                    latex:inline($config, ., ("tei-editor3"), .)
                                )
                    ...
                    case element(ref) return
                        if (@ana) then
                            ...
                            latex:inline($config, ., ("tei-ref1", "itinerary"), .)
                        else
                            if (not(@target)) then
                                latex:inline($config, ., ("tei-ref2"), .)
                            else
                                if (not(node())) then
                                    latex:link($config, ., ("tei-ref3"), @target, @target, map {})
                                else
                                    latex:link($config, ., ("tei-ref4"), ., @target, map {})
                    ...
                    case element(bibl) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-bibl1"), $content)
                        else
                            if (parent::listBibl) then
                                latex:listItem($config, ., ("tei-bibl2"), ., ())
                            else
                    ...
                    case element(titleStmt) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-titleStmt1", "editors"), $content)
                        else
                    ...
                    case element(sourceDesc) return
                        ...
                        return
                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-sourceDesc"), $content)
                    case element(msDesc) return
                        ...
                        return
                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-msDesc"), $content)
                    case element(person) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-person1", "person-entry"), $content)
                        else
                    ...
                    case element(place) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-place1"), $content)
                        else
                            latex:inline($config, ., ("tei-place2"), .)
                    case element() return
