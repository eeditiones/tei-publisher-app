    ...
    $input !         (
            ...
            return
                    ...
                    case element(hi) return
                        if (@rend) then
                            ...
                            if (@rend='b') then
                                ...
                                if (@rendition) then
                                    html:inline($config, ., css:get-rendition(., ("tei-hi3", css:map-rend-to-class(.))), .)
                                else
                    ...
                    case element(note) return
                        if (@target) then
                            html:note($config, ., ("tei-note1", css:map-rend-to-class(.)), ., (), let $nr := count(./preceding::note[@target]) let $ch := codepoints-to-string(string-to-codepoints("a") + $nr mod 26) return  '-' || $ch)
                        else
                            if (@place) then
